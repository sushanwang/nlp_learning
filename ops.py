...
def highway(input_, size, layer_size=1, bias=-2, f=tf.nn.relu):
  ...
  for idx in xrange(layer_size):
    output = f(tf.contrib.layers.fully_connected(inputs = output, num_outputs = int(size), activation_fn = None))

    transform_gate = tf.sigmoid(
        tf.contrib.layers.fully_connected(inputs = input_, num_outputs = int(size), biases_initializer = bias, activation_fn = None))
