  ...
  def prepare_model(self):
    with tf.variable_scope("LSTMTDNN"):
      ...
      with tf.variable_scope("CNN") as scope:
        ...
        ###fixed
        char_indices = tf.split(self.char_inputs, self.seq_length, 1)
        word_indices = tf.split(tf.expand_dims(self.word_inputs, -1), self.seq_length, 1)
      ...
      with tf.variable_scope("LSTM") as scope:
        ...
        loss = 0
        true_outputs = tf.split(self.true_outputs, self.seq_length, 1)
        ...
        for idx, (top_h, true_output) in enumerate(zip(outputs, true_outputs)):
          ...
          if self.hsm > 0:
            ...
            if idx != 0:
              scope.reuse_variables()
            proj = tf.contrib.layers.fully_connected(inputs = top_h, num_outputs = self.word_vocab_size, activation_fn = None)
